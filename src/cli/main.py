...
def main():
    ...
    # Search Vectors
    parser.add_argument(
        "-s", "--symptom", help="Buscar por nombre de síntoma/dolencia (ej: acné)"
    )
    parser.add_argument(
        "-z", "--zone", help="Buscar por zona del cuerpo (ej: cara, estómago, piel)"
    )
    parser.add_argument(
        "-d",
        "--description",
        help="Buscar por palabras clave en la descripción (ej: miedo, separación)",
    )
    ...
    parser.add_argument(
        ...
        help="Ruta al archivo JSON de datos",
    )
    ...
    results = engine.multi_vector_search(
        symptom=args.symptom, zone=args.zone, description=args.description
    )
