...
def _get_interpretations(content: str, header_val: str) -> list[dict]:
    ...
    if len(parts) > 1:
        for i in range(1, len(parts), 2):
            ...
            author_name = auth_map.get(
                raw_author_name.upper(), raw_author_name
            )
            author_content = (
...
def _apply_global_fallback(
    ...
    for interp in mapped_item["interpretaciones"]:
        if (
            not interp["conflicto_emocional"]
            and not interp["modelo_mental"]
        ):
            ...
            if target_text.strip():
                ...
                content_part = header_pattern.sub(
                    "", content_part
                ).strip()
